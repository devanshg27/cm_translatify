...
mkdir ${SCRATCH_DIR}/mbart.cc25
cp ${HIEN_SCRATCH_DIR}/trimmed/dict.txt ${SCRATCH_DIR}/mbart.cc25/
cp ${HIEN_SCRATCH_DIR}/checkpoint/checkpoint_3_32000.pt ${SCRATCH_DIR}/mbart.cc25/model.pt # choose best val loss
cp ${HIEN_SCRATCH_DIR}/mbart.cc25/sentence.bpe.model ${SCRATCH_DIR}/mbart.cc25/

# 1: Parse the datasets

python3 code1.py $SCRATCH_DIR $DATA_DIR

# 2: run spm_encode on the data

for Item in 'train' 'valid' 'test' 'testph' 'iitbtrai' 'iitbvali' 'iitbtest';
    do
        spm_encode --model=${SCRATCH_DIR}/mbart.cc25/sentence.bpe.model < ${SCRATCH_DIR}/preprocessed/${Item}.${SRC} > ${SCRATCH_DIR}/preprocessed/${Item}.spm.${SRC}
        spm_encode --model=${SCRATCH_DIR}/mbart.cc25/sentence.bpe.model < ${SCRATCH_DIR}/preprocessed/${Item}.${TGT} > ${SCRATCH_DIR}/preprocessed/${Item}.spm.${TGT}
    done

# 3: Build vocabulary for trimming

mkdir $SCRATCH_DIR/trimmed
cp ${SCRATCH_DIR}/mbart.cc25/dict.txt $SCRATCH_DIR/trimmed/dict.txt

# 4: trim mBART

cp ${SCRATCH_DIR}/mbart.cc25/model.pt $SCRATCH_DIR/trimmed/model.pt
...
--destdir ${SCRATCH_DIR}/postprocessed/hi-en \
--thresholdtgt 0 \
--thresholdsrc 0 \
...
fairseq-train ${SCRATCH_DIR}/postprocessed/hi-en  --encoder-normalize-before --decoder-normalize-before \
